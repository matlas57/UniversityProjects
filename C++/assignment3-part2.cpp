...
class BlackJackGame
{
    public:
        ...
        vector<HumanPlayer> m_humans; //vector container for the human players
        

        //Constructor
        BlackJackGame(int numHumans) 
        {
            m_deck = new Deck();
        ...
        void play()
        {
            ...
            vector<Hand> humanHands;
            vector<int> finalTotals; // this vector will store all the hands final hand values which will be used to find the winner
            for (int i = 0; i < m_humans.size(); i++)
            {
                Hand h = *m_humans[i].h;
                humanHands.push_back(h);
            }
            for (int i = 0; i < 2; i++)
            {
                for (int j = 0; j < humanHands.size(); j++)
                {
                    m_deck->deal(humanHands[j]);
                }
                m_deck->deal(cHand);
            }
            ...
            for (int i = 0; i < humanHands.size(); i++)
            {
                cout << "Player " << i + 1 << ": ";
                for (int j = 0; j < humanHands[i].hand.size(); j++)
                {
                    humanHands[i].hand[j].displayCard();
                    cout << " ";
                }
                cout << "[" << humanHands[i].getTotal() << "]" << endl;
            }

            for (int i = 0; i < m_humans.size(); i++)
            {
                cout << "Player " << i + 1 << "'s turn" << endl; 
                while (m_humans[i].isDrawing())
                {
                    m_deck->deal(humanHands[i]); //deal a card
                    ...
                    else if (humanHands[i].getTotal() == 21)
                    {
                        break;
                    }
                }
                finalTotals.push_back(humanHands[i].getTotal());
            }
            //casino drawing turn
            ...
            finalTotals.push_back(cHand.getTotal());
            //find the winner
            vector<int> sortedTotals = finalTotals; //copy the finalTotals vector so it can be sorted
            sort(sortedTotals.begin(), sortedTotals.end(), greater<int>()); //perform the sorting
            int winningValue = 0;
            for (int i = 0; i < sortedTotals.size() - 1; i++)
            {
                // if the two greatest, non-busted hands are equal then we have a push situation
                if (sortedTotals[i] <= 21 && sortedTotals[i] == sortedTotals[i+1])
                {
                    cout << "Push: no one wins" << endl;
                    m_deck->deck.clear();
                    return;
                }
                // if theres no push situation store the value of the winning hand
                else if (sortedTotals[i] <= 21)
                {
                    winningValue = sortedTotals[i];
                    break;
                }
            }
            /*
             * find who wins using the winning value
             * go through the unsorted array and compare the values to the winning value and use the index found to determine the winner
             */
            for (int i = 0; i < finalTotals.size(); i++)
            {
                // when the index is found
                if (finalTotals[i] == winningValue)
                {
                    // casino value is at index 3
                    if (i == 3)
                    {
                        casinoWins++;
                        cout << "Casino wins" << endl;
                    }
                    // if a player wins print their player number
                    else 
                    {
...
int main()
{
    ...
    {
        cout << "How many players? (enter a number between 1 and 3)" << endl;
        cin >> numPlayers;
    }
